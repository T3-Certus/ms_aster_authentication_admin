...
export async function loginUser(req: any, res: Response<GenericServiceResponse | GenericServiceErrorResponse>){
  ...
  try {
    ...
    if(!user){
      res.status(401).json(status401Unauthorized("Invalid email"))  
    }
    ...
    if(!verifiedPassword){
      res.status(401).json(status401Unauthorized("Invalid password"))
    }

    const {accessToken, refreshToken} = await generateTokens(userData)
    
    res.status(200).json(status200Ok({accessToken: accessToken, refreshToken: refreshToken}, "", "User successfully logged"))
